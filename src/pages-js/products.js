  ...
  <div class="max-w-7xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p id="products-count" class="text-sm text-slate-500 mt-0.5"></p>
      </div>
      <button id="add-product-btn"
        class="bg-orange-600 hover:bg-orange-700 text-white px-4 py-2.5 rounded-xl text-sm font-semibold transition-colors flex items-center gap-2 shadow-sm">
...
function stockBadge(stock) {
  if (stock === 0) return `<span class="text-xs font-medium bg-red-50 text-red-600 px-2 py-0.5 rounded-full">Out of stock</span>`;
  if (stock < 10) return `<span class="text-xs font-medium bg-amber-50 text-amber-700 px-2 py-0.5 rounded-full">Low: ${stock}</span>`;
  return `<span class="text-xs font-medium bg-emerald-50 text-emerald-700 px-2 py-0.5 rounded-full">${stock} in stock</span>`;
}
...
export async function init() {
  ...
  function renderGrid() {
    ...
    if (paginated.length === 0) {
      ...
      const catCls = (cat) => CATEGORY_COLORS[cat] || 'bg-slate-100 text-slate-600';
      gridEl.innerHTML = `
        <div class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          ${paginated.map(p => `
            <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 group">
              <!-- Image -->
              <div class="relative overflow-hidden bg-slate-50 cursor-pointer" data-navigate="/products/${p.id}">
                ...
                <div class="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                  <button data-action="edit" data-id="${p.id}"
                    class="bg-white text-slate-700 hover:bg-orange-600 hover:text-white px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors shadow-sm">
                    Edit
                  </button>
                  <button data-action="delete" data-id="${p.id}"
                    class="bg-white text-slate-700 hover:bg-red-500 hover:text-white px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors shadow-sm">
                    Delete
                  </button>
                </div>
              </div>
              ...
              <div class="p-4">
                ...
                <h3 class="font-semibold text-slate-900 text-sm line-clamp-1 leading-snug mb-3 cursor-pointer hover:text-orange-600 transition-colors"
